...
#ifndef NYAS_MATHC_TMP
#define NYAS_MATHC_TMP
...
constexpr float *vec3_cross(float *result, const float *v0,
							const float *v1)
{
...
namespace nym
{
	...
	constexpr float vec2_t::magnitude(vec2_t vec)
	{
		return sqrtf(dot(vec, vec));
	}

	constexpr vec2_t vec2_t::normal(vec2_t vec)
	{
		return vec / magnitude(vec);
	}

	constexpr vec2_t::operator float *()
	{ return &x; }

	constexpr vec2_t::operator const float *() const
	{ return &x; }

	constexpr float vec2_t::operator[](int i) const
	{ return (&x)[i]; }

	constexpr float& vec2_t::operator[](int i)
	{ return (&x)[i]; }
	...
	constexpr inline void vec2_t::operator*=(float k)
	{ *this = *this * k; }
	...
	constexpr vec3_t vec3_t::normal(vec3_t vec)
	{
		return vec / magnitude(vec);
	}

	constexpr vec3_t::operator float *()
	{ return &x; }

	constexpr vec3_t::operator const float *() const
	{ return &x; }

	constexpr float vec3_t::operator[](int i) const
	{ return (&x)[i]; }

	constexpr float& vec3_t::operator[](int i)
	{ return (&x)[i]; }
	...
	constexpr inline void vec3_t::operator*=(float k)
	{ *this = *this * k; }
...
#ifndef NYAS_MATH_VIEJO_H
#define NYAS_MATH_VIEJO_H
...
struct NyVec3
{
	...
	inline operator nym::vec3_t()
	{ return std::move(*(nym::vec3_t *) this); }
};
