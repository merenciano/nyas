...
struct vec3_t {
  ...
  void operator+=(const vec3_t &other);
  vec3_t operator-(const vec3_t &other) const;
  void operator-=(const vec3_t &other);
  vec3_t operator*(const vec3_t &other) const;
  void operator*=(const vec3_t &other);
  vec3_t operator*(float k) const;
...
inline void vec3_t::operator+=(const vec3_t &other) { *this = *this + other; }
...
inline void vec3_t::operator*=(float k) { *this = *this * k; }
inline vec3_t vec3_t::operator*(float k) const {
  return {x * k, y * k, z * k};
}
...
struct NyVec3 {
  ...
  inline operator float *() { return &X; }
  inline operator nym::vec3_t() { return std::move(*(nym::vec3_t*)this); }
};
