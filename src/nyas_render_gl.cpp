...
static inline _GL_Format _GL_TexFmt(NyasTexFmt Format) {
    ...
    case NyasTexFmt_DepthStencil:
        return {GL_DEPTH24_STENCIL8, GL_DEPTH_COMPONENT, GL_FLOAT};
    default:
        printf("Unrecognized texture format: (%d).", Format);
        return {0, 0, 0};
    }
}
...
static GLenum _GL_ShaderType(NyasShaderStage stage) {
    switch (stage) {
    case NyasShaderStage_Vertex:
        return GL_VERTEX_SHADER;
    case NyasShaderStage_Fragment:
...
void _NySetTex(NyResourceID id, NyasTexture tex, NyasTexImage img, NyasTexInfo info, int faces) {
    ...
    for (int i = 0; i < faces; ++i) {
        glTextureSubImage3D(
            id, img.Level, 0, 0, tex.Layer * faces + i, info.Width >> img.Level,
...
void _NyBuildPipeline(NyResourceID id, NyasPipelineBuilder *pb) {
    ...
    for (int i = 0; i < NyasShaderStage_COUNT; ++i) {
        ...
        glShaderSource(
            shader_ids[i], pb->Source[i].SrcStr.size(), pb->Source[i].SrcStr.data(),
            pb->Source[i].SrcLen.data());
...
void _NyCreateFramebuf(NyasFramebuffer *fb) { glGenFramebuffers(1, &fb->Resource.ID); }
...
void _NyUseFramebuf(uint32_t id) { glBindFramebuffer(GL_FRAMEBUFFER, id); }

void _NyReleaseFramebuf(NyasFramebuffer *fb) { glDeleteFramebuffers(1, &fb->Resource.ID); }
...
void _NyDraw(NyasDrawElementCmd *commands, int count) {
    ...
    glBufferData(GL_DRAW_INDIRECT_BUFFER, sizeof(NyasDrawElementCmd) * count,
                 (const void *)commands, GL_DYNAMIC_DRAW);

    glMultiDrawElementsIndirect(
        GL_TRIANGLES, sizeof(NyDrawIdx) == 4 ? GL_UNSIGNED_INT : GL_UNSIGNED_SHORT, 0, count, 0);
}
...
void _NyEnableBlend(void) { glEnable(GL_BLEND); }

void _NyDisableBlend(void) { glDisable(GL_BLEND); }

void _NyEnableCull(void) { glEnable(GL_CULL_FACE); }

void _NyDisableCull(void) { glDisable(GL_CULL_FACE); }

void _NyEnableDepthTest(void) { glEnable(GL_DEPTH_TEST); }

void _NyDisableDepthTest(void) { glDisable(GL_DEPTH_TEST); }

void _NyEnableDepthMask(void) { glDepthMask(GL_TRUE); }

void _NyDisableDepthMask(void) { glDepthMask(GL_FALSE); }
...
void _NySetDepthFunc(NyasDepthFunc depth_func) {
    ...
    if (gl_value) {
        NYAS_ASSERT(gl_value != (GLenum)NyasError_SwitchBadLabel);
        glDepthFunc(gl_value);
    }
}

void _NyEnableStencilTest(void) { glEnable(GL_STENCIL); }

void _NyDisableStencilTest(void) { glDisable(GL_STENCIL); }

void _NyEnableStencilMask(void) { glStencilMask(GL_TRUE); }

void _NyDisableStencilMask(void) { glStencilMask(GL_FALSE); }

void _NyEnableScissor(void) { glEnable(GL_SCISSOR_TEST); }
