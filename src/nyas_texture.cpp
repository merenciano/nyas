...
NyasTexture NyTextures::Alloc(NyasTexInfo info, NyasTexFlags flags) {
    if (flags & NyasTexFlags_Cubemap) {
        ...
        NYAS_ASSERT(false && "Out of cubemap memory. Consider increasing "
                             "NYAS_CUBEMAP_ARRAYS or NYAS_CUBEMAP_ARRAY_SIZE.");
    } else {
        ...
        NYAS_ASSERT(false && "Out of tex memory. Consider increasing "
                             "NYAS_TEX_ARRAYS or NYAS_TEX_ARRAY_SIZE.");
    }
}
...
void NyTextures::Load(NyasTexture handle, const char *path) {
    ...
    Updates.emplace_back(
        handle, std::async(std::launch::async, LoadImg, path, Textures[handle.Index].Info.Format));
}
        ...
        if (tex.Flags & NyasTexFlags_Cubemap) {
            if (img.Data[0]) {
                ...
                nyas::render::_NyAllocTex(
                    CubemapInternalIDs[tex.Index], Cubemaps[tex.Index].Info,
                    NYAS_CUBEMAP_ARRAY_SIZE * 6);
            }
        } else {
            if (img.Data[0]) {
                nyas::render::_NySetTex(
