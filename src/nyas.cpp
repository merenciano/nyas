...
// #define STBI_REALLOC nyas::nyas_realloc
...
// #define TINYOBJ_REALLOC nyas::nyas_realloc
// #define TINYOBJ_CALLOC nyas::nyas_calloc
...
typedef pthread_t NyThread;

typedef int NyasSchedState;
enum NyasSchedState_ {
  NyasSchedState_None = 0,
...
struct NySched {
  NySched *next;
  NyArray<NyJob> queue;
  NyArray<NyThread> threads;
  pthread_mutex_t mtx;
  pthread_cond_t cond;
  int waiting;
  NyasSchedState state;

  NySched() : next(NULL), waiting(0), state(NyasSchedState_None) {}
};

// ---
// [PRIVATE]
// ---
void nypx_tex_create(NyasTexture *t);
void nypx_tex_set(NyasTexture *t);
void nypx_tex_release(uint32_t *id);

void nypx_mesh_create(uint32_t *id, uint32_t *vid, uint32_t *iid);
void nypx_mesh_use(NyasMesh *m, NyasShader *s);
void nypx_mesh_set(NyasMesh *mesh, uint32_t shader_id);
...
void nypx_fb_create(NyasFramebuffer *fb);
void nypx_fb_set(uint32_t fb_id, uint32_t tex_id, struct NyasTexTarget *tt);
void nypx_fb_use(uint32_t id);
void nypx_fb_release(NyasFramebuffer *fb);
...
void nypx_viewport(NyRect rect);
void nypx_scissor(NyRect rect);
...
static void nyas__scrollcallback(GLFWwindow *window, double x_offset,
                                 double y_offset) {
  NY_UNUSED(window);
...
bool InitIO(const char *title, int win_w, int win_h) {
  ...
  glfwSetCursorEnterCallback((GLFWwindow *)G_Ctx->IO.InternalWindow,
                             nyas__cursor_enter_callback);
  glfwSetWindowFocusCallback((GLFWwindow *)G_Ctx->IO.InternalWindow,
                             nyas__window_focus_callback);
  glfwSetInputMode((GLFWwindow *)G_Ctx->IO.InternalWindow, GLFW_STICKY_KEYS,
                   GLFW_TRUE);
  glfwSetInputMode((GLFWwindow *)G_Ctx->IO.InternalWindow, GLFW_STICKY_MOUSE_BUTTONS,
                   GLFW_TRUE);
  ...
  G_Ctx->Cfg.NavCfg.Speed = 10.0f;
  G_Ctx->Cfg.NavCfg.Sensitivity = 0.001f;
  G_Ctx->Cfg.NavCfg.ScrollSensitivity = 1.0f;
...
void PollIO(void) {
  ...
  glfwGetWindowSize((GLFWwindow *)G_Ctx->IO.InternalWindow, &G_Ctx->IO.WindowSize.X,
                    &G_Ctx->IO.WindowSize.Y);

  if (G_Ctx->IO.ShowCursor) {
    glfwSetInputMode((GLFWwindow *)G_Ctx->IO.InternalWindow, GLFW_CURSOR,
                     GLFW_CURSOR_NORMAL);
  } else {
    glfwSetInputMode((GLFWwindow *)G_Ctx->IO.InternalWindow, GLFW_CURSOR,
...
static inline void nyas__check_handle(int h, void *arr) {
  (void)h, (void)arr;
  NYAS_ASSERT(arr && h >= 0 && ((NyPool<NyasTexture> *)arr)->Arr.Size > h &&
              "Invalid handle range.");
}

static void nyas__file_reader(void *_1, const char *path, int _2,
                              const char *_3, char **buf, size_t *size) {
...
static int _TexFaces(NyasTexType type) {
  switch (type) {
  case NyasTexType_2D:
  case NyasTexType_Array2D:
    return 1;
  case NyasTexType_Cubemap:
  case NyasTexType_ArrayCubemap:
    return 6;
  default:
    return 0;
  }
...
void LoadTexture(NyasHandle texture, struct NyasTexDesc *desc,
                 const char *path) {
  ...
  stbi_set_flip_vertically_on_load(t->Data.Flags &
                                   NyasTexFlags_FlipVerticallyOnLoad);
  ...
  for (int i = 0; i < face_count; ++i) {
    ...
    if (_TexFmtFloat(t->Data.Format)) {
      img.Pix =
          stbi_loadf(p, &t->Data.Width, &t->Data.Height, &channels, fmt_ch);
    } else {
      img.Pix =
          stbi_load(p, &t->Data.Width, &t->Data.Height, &channels, fmt_ch);
    }
...
NyasHandle CreateShader(const struct NyasShaderDesc *desc) {
  ...
  Shaders[ret].Shared =
      NYAS_ALLOC((desc->SharedDataCount + desc->SharedTexCount +
                  desc->SharedCubemapCount) *
                 sizeof(float));
  return ret;
}
...
static void _SetMeshMsh(NyasMesh *mesh, const char *path) {
  ...
  mesh->Attribs = NyasVtxAttribFlags_Position | NyasVtxAttribFlags_Normal |
                  NyasVtxAttribFlags_Tangent | NyasVtxAttribFlags_Bitangent |
                  NyasVtxAttribFlags_UV;
  mesh->VtxSize = *(size_t *)data; // *(((size_t *)data)++)
  data += sizeof(size_t);
  mesh->Vtx = (float *)NYAS_ALLOC(mesh->VtxSize);
  ...
  mesh->ElementCount =
      (*(size_t *)data) / sizeof(NyDrawIdx); // *(((size_t *)data)++)
  data += sizeof(size_t);
  mesh->Indices =
      (NyDrawIdx *)NYAS_ALLOC(mesh->ElementCount * sizeof(NyDrawIdx));
...
static void _SyncShaderData(NyasShader *s, void *srcdata, int common) {
  ...
  // set opengl uniforms
  if (dc) {
    nypx_shader_set_data(dl, (float *)srcdata, dc / 4);
  }
...
void _SyncShader(NyasShader *s) {
  ...
  if (s->Resource.Flags & NyasResourceFlags_Dirty) {
    ...
    nypx_shader_compile(s->Resource.Id, s->Name);
    nypx_shader_loc(s->Resource.Id, &s->Location[0].Data, &uniforms[0], 6);
...
static void _SyncFramebuf(NyasHandle framebuffer) {
  ...
  if (fb->Resource.Flags & NyasResourceFlags_Dirty) {
    for (int i = 0; fb->Target[i].Tex != NyasCode_None; ++i) {
      ...
      nypx_fb_set(fb->Resource.Id, t->Resource.Id, &fb->Target[i]);
    }
...
void Draw(NyasDrawCmd *cmd) {
  ...
  if (cmd->ShaderMaterial.Shader != NyasCode_NoOp) {
    ...
    _SyncShader(s);
    nypx_shader_use(s->Resource.Id);
    _SyncShaderData(s, cmd->ShaderMaterial.Ptr, true);
  }

  NyasDrawState &s = cmd->State;
  nypx_clear_color(s.BgColorR, s.BgColorG, s.BgColorB, s.BgColorA);
  nypx_clear(s.EnableFlags & NyasDrawFlags_ColorClear,
             s.EnableFlags & NyasDrawFlags_DepthClear,
             s.EnableFlags & NyasDrawFlags_StencilClear);

  nypx_viewport(
      {s.ViewportMinX, s.ViewportMinY, s.ViewportMaxX, s.ViewportMaxY});
  nypx_scissor({s.ScissorMinX, s.ScissorMinY, s.ScissorMaxX, s.ScissorMaxY});
  ...
  nypx_depth_set(s.Depth);
  nypx_blend_set(s.BlendSrc, s.BlendDst);
  nypx_cull_set(s.FaceCulling);

  for (int i = 0; i < cmd->UnitCount; ++i) {
    NyasMesh *imsh = &Meshes[cmd->Units[i].Mesh];
    nyas__check_handle(cmd->Units[i].Mesh, &Meshes);
    ...
    nypx_draw(imsh->ElementCount, sizeof(NyDrawIdx) == 4);
  }
}
} // namespace Nyas

NyasHandle NYAS_UTILS_SPHERE;
NyasHandle NYAS_UTILS_CUBE;
NyasHandle NYAS_UTILS_QUAD;
...
void NyasCamera::Navigate() {
  ...
  float speed = Nyas::G_Ctx->Cfg.NavCfg.Speed * Nyas::G_Ctx->Platform.DeltaTime;

  // Rotation
  if (Nyas::G_Ctx->IO.MouseButton[NyasMouseButton_Right] == NyasKeyState_DOWN) {
    mouse_down_pos = Nyas::G_Ctx->IO.MousePosition;
  }

  float tmp_vec[3];
  if (Nyas::G_Ctx->IO.MouseButton[NyasMouseButton_Right] == NyasKeyState_PRESSED) {
    NyVec2 curr_pos = Nyas::G_Ctx->IO.MousePosition;
    NyVec2 offset = {
        (curr_pos.X - mouse_down_pos.X) * Nyas::G_Ctx->Cfg.NavCfg.Sensitivity,
        (mouse_down_pos.Y - curr_pos.Y) * Nyas::G_Ctx->Cfg.NavCfg.Sensitivity};

    vec3_add(fwd, fwd,
             vec3_multiply_f(tmp_vec, vec3_cross(tmp_vec, NyVec3::Up(), fwd),
                             -offset.X));
  ...
  // Position
  if (Nyas::G_Ctx->IO.Keys[NyasKey_W] == NyasKeyState_PRESSED) {
    vec3_add(eye, eye, vec3_multiply_f(tmp_vec, fwd, speed));
  }

  if (Nyas::G_Ctx->IO.Keys[NyasKey_S] == NyasKeyState_PRESSED) {
    vec3_add(eye, eye, vec3_multiply_f(tmp_vec, fwd, -speed));
  }

  if (Nyas::G_Ctx->IO.Keys[NyasKey_A] == NyasKeyState_PRESSED) {
    vec3_add(eye, eye,
             vec3_multiply_f(tmp_vec, vec3_cross(tmp_vec, NyVec3::Up(), fwd),
                             speed));
  }

  if (Nyas::G_Ctx->IO.Keys[NyasKey_D] == NyasKeyState_PRESSED) {
    vec3_add(eye, eye,
             vec3_multiply_f(tmp_vec, vec3_cross(tmp_vec, NyVec3::Up(), fwd),
                             -speed));
  }

  if (Nyas::G_Ctx->IO.Keys[NyasKey_Space] == NyasKeyState_PRESSED) {
    vec3_add(eye, eye, vec3_multiply_f(tmp_vec, NyVec3::Up(), speed));
  }

  if (Nyas::G_Ctx->IO.Keys[NyasKey_LeftShift] == NyasKeyState_PRESSED) {
    vec3_add(eye, eye, vec3_multiply_f(tmp_vec, NyVec3::Up(), -speed));
  }
  ...
  // Zoom
  if (Nyas::G_Ctx->IO.MouseScroll.Y != 0.0f) {
    Fov -= Nyas::G_Ctx->IO.MouseScroll.Y * Nyas::G_Ctx->Cfg.NavCfg.ScrollSensitivity;
    Fov = clampf(Fov, 1.0f, 120.0f);
    mat4_perspective(
        Proj, to_radians(Fov),
        (float)Nyas::G_Ctx->IO.WindowSize.X / (float)Nyas::G_Ctx->IO.WindowSize.Y, 0.1f, Far);
  }
}
...
NySched *CreateScheduler(int thread_count) {
  ...
  NySched *s = (NySched *)NYAS_ALLOC(sizeof(struct NySched));
  *s = NySched();
  ...
  for (int i = 0; i < thread_count; ++i) {
    NyThread thread;
    if (pthread_create(&thread, NULL, _Worker, s)) {
  ...
  s->state = NyasSchedState_Running;
...
void WaitJobs(NySched *s) {
  ...
  while (s->queue.Size || s->waiting != s->threads.Size) {
...
int DestroySched(NySched *s) {
  pthread_mutex_lock(&s->mtx);
  s->state = NyasSchedState_Closing;
  pthread_cond_broadcast(&s->cond);
  pthread_mutex_unlock(&s->mtx);
...
AssetLoader *nyut_assets_create(void) {
  AssetLoader *l = (AssetLoader *)NYAS_ALLOC(sizeof(struct AssetLoader));
  l->Async = {};
  l->Sequential = {};
  return l;
}
...
static void _MeshLoader(void *arg) {
  ...
  *a->Mesh = Nyas::LoadMesh(
      a->Path); // TODO: separate create and load like textures in
...
void AssetLoader::AddMesh(MeshLoaderArgs *args) {
  Async.Push({_MeshLoader, args});
}

void AssetLoader::AddTex(TexLoaderArgs *args) {
  Async.Push({_TexLoader, args});
}

void AssetLoader::AddShader(ShaderLoaderArgs *args) {
  Sequential.Push({_ShaderLoader, args});
}

void AssetLoader::AddEnv(EnvLoaderArgs *args) {
  Async.Push({_EnvLoader, args});
}

void AssetLoader::AddJob(NyJob j, bool async) {
  if (async) {
    Async.Push(j);
  } else {
    Sequential.Push(j);
...
void AssetLoader::Load(int threads) {
...
void nyut_mesh_init_geometry(void) {
  NYAS_UTILS_SPHERE = Nyas::CreateMesh();
  NYAS_UTILS_CUBE = Nyas::CreateMesh();
  NYAS_UTILS_QUAD = Nyas::CreateMesh();
  nyut_mesh_set_geometry(NYAS_UTILS_SPHERE, NyasGeometry_Sphere);
  nyut_mesh_set_geometry(NYAS_UTILS_CUBE, NyasGeometry_Cube);
  nyut_mesh_set_geometry(NYAS_UTILS_QUAD, NyasGeometry_Quad);
}

void nyut_mesh_set_geometry(NyasHandle msh, NyasGeometry geo) {
...
void nyut_env_load(const char *path, NyasHandle *lut, NyasHandle *sky,
                   NyasHandle *irr, NyasHandle *pref) {
  FILE *f = fopen(path, "r");
  char hdr[9];
  fread(hdr, 8, 1, f);
  hdr[8] = '\0';
...
struct NyGL_TexDesc {
  ...
  NyGL_TexDesc(GLenum tg, GLint min, GLint mag, GLint ws, GLint wt, GLint wr,
               float br, float bg, float bb, float ba)
      : target(tg), min_f(min), mag_f(mag), ws(ws), wt(wt), wr(wr), br(br),
        bg(bg), bb(bb), ba(ba) {}
};

static GLenum glttarget(NyasTexType type) {
  switch (type) {
  case NyasTexType_2D:
...
static GLint gltfilter(NyasTexFilter f) {
  switch (f) {
  case NyasTexFilter_Linear:
    return GL_LINEAR;
  case NyasTexFilter_LinearMipmapLinear:
    return GL_LINEAR_MIPMAP_LINEAR;
  case NyasTexFilter_LinearMipmapNearest:
    return GL_LINEAR_MIPMAP_NEAREST;
  case NyasTexFilter_Nearest:
    return GL_NEAREST;
  case NyasTexFilter_NearestMipmapNearest:
    return GL_NEAREST_MIPMAP_NEAREST;
  case NyasTexFilter_NearestMipmapLinear:
...
static GLint gltwrap(NyasTexWrap w) {
  switch (w) {
  case NyasTexWrap_Repeat:
    return GL_REPEAT;
  case NyasTexWrap_Clamp:
    return GL_CLAMP_TO_EDGE;
  case NyasTexWrap_Mirror:
    return GL_MIRRORED_REPEAT;
  case NyasTexWrap_Black:
...
static NyGL_TexDesc gltdesc(NyasTexDesc *d) {
  return {glttarget(d->Type), gltfilter(d->MinFilter), gltfilter(d->MagFilter),
          gltwrap(d->WrapS),  gltwrap(d->WrapT),       gltwrap(d->WrapR),
          d->BorderColor[0],  d->BorderColor[1],       d->BorderColor[2],
          d->BorderColor[3]};
}
...
static struct gltfmt_result gltfmt(NyasTexFmt fmt) {
  switch (fmt) {
  case NyasTexFmt_R_8:
...
void nypx_tex_create(struct NyasTexture *t) {
  NyGL_TexDesc d = gltdesc(&t->Data);
...
void nypx_tex_set(struct NyasTexture *t) {
  GLenum type = glttarget(t->Data.Type);
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(type, t->Resource.Id);
  struct gltfmt_result fmt = gltfmt(t->Data.Format);
  for (int i = 0; i < t->Img.Size; ++i) {
    ...
    glTexImage2D(target, t->Img[i].MipLevel, fmt.ifmt, w, h, 0, fmt.fmt,
                 fmt.type, t->Img[i].Pix);
  }
...
void nypx_tex_release(uint32_t *id) { glDeleteTextures(1, id); }
...
void nypx_shader_create(uint32_t *id) { *id = glCreateProgram(); }
...
static void nypx__set_tex(int loc, int *t, int c, int unit, GLenum target) {
  for (int i = 0; i < c; ++i) {
...
void nypx_shader_use(uint32_t id) { glUseProgram(id); }

void nypx_shader_release(uint32_t id) { glDeleteProgram(id); }

void nypx_fb_create(struct NyasFramebuffer *fb) {
  glGenFramebuffers(1, &fb->Resource.Id);
}

int nypx__fb_attach_gl(NyasFbAttach a) {
  switch (a) {
  case NyasFbAttach_Depth:
    return GL_DEPTH_ATTACHMENT;
  case NyasFbAttach_Stencil:
    return GL_STENCIL_ATTACHMENT;
  case NyasFbAttach_DepthStencil:
    return GL_DEPTH_STENCIL_ATTACHMENT;
  default:
    return GL_COLOR_ATTACHMENT0 + a;
  }
}

void nypx_fb_set(uint32_t fb_id, uint32_t tex_id, struct NyasTexTarget *tt) {
  glBindFramebuffer(GL_FRAMEBUFFER, fb_id);
  GLenum face = tt->Face == NyasTexFace_2D
                    ? GL_TEXTURE_2D
...
void nypx_fb_use(uint32_t id) { glBindFramebuffer(GL_FRAMEBUFFER, id); }

void nypx_fb_release(struct NyasFramebuffer *fb) {
...
void nypx_blend_enable(void) { glEnable(GL_BLEND); }

void nypx_blend_disable(void) { glDisable(GL_BLEND); }

void nypx_cull_enable(void) { glEnable(GL_CULL_FACE); }

void nypx_cull_disable(void) { glDisable(GL_CULL_FACE); }

void nypx_depth_enable_test(void) { glEnable(GL_DEPTH_TEST); }

void nypx_depth_disable_test(void) { glDisable(GL_DEPTH_TEST); }

void nypx_depth_enable_mask(void) { glDepthMask(GL_TRUE); }

void nypx_depth_disable_mask(void) { glDepthMask(GL_FALSE); }

static GLenum nypx__gl_blend(int blend_func) {
  switch (blend_func) {
  case NyasBlendFunc_Default:
    return 0xFFFF;
  case NyasBlendFunc_One:
    return GL_ONE;
  case NyasBlendFunc_SrcAlpha:
    return GL_SRC_ALPHA;
  case NyasBlendFunc_OneMinusSrcAlpha:
    return GL_ONE_MINUS_SRC_ALPHA;
  case NyasBlendFunc_Zero:
    return GL_ZERO;
  default:
    NYAS_ASSERT(false);
    return -1;
...
static GLenum nypx__gl_cull(NyasFaceCull cull) {
  switch (cull) {
  case NyasFaceCull_Back:
    return GL_BACK;
  case NyasFaceCull_Front:
    return GL_FRONT;
  case NyasFaceCull_FrontAndBack:
    return GL_FRONT_AND_BACK;
  case NyasFaceCull_Default:
    return 0;
  default:
    return NyasError_SwitchBadLabel;
  }
}
...
static GLenum nypx__gl_depth(NyasDepthFunc df) {
  switch (df) {
  case NyasDepthFunc_LessEqual:
    return GL_LEQUAL;
  case NyasDepthFunc_Less:
    return GL_LESS;
  case NyasDepthFunc_Default:
    return 0;
  default:
    return NyasError_SwitchBadLabel;
  }
...
void nypx_stencil_enable_test(void) { glEnable(GL_STENCIL); }

void nypx_stencil_disable_test(void) { glDisable(GL_STENCIL); }

void nypx_stencil_enable_mask(void) { glStencilMask(GL_TRUE); }

void nypx_stencil_disable_mask(void) { glStencilMask(GL_FALSE); }

void nypx_scissor_enable(void) { glEnable(GL_SCISSOR_TEST); }

void nypx_scissor_disable(void) { glDisable(GL_SCISSOR_TEST); }
