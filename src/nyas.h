...
void SetFramebufferTarget(NyasHandle fb, int index,
...
typedef struct NyasTexDesc {
  ...
  NyasTexDesc()
      : Width(0), Height(0), Flags(NyasTexFlags_None), Type(NyasTexType_2D),
        Format(NyasTexFmt_SRGB_8), MinFilter(NyasTexFilter_Linear),
        MagFilter(NyasTexFilter_Linear), WrapS(NyasTexWrap_Repeat),
  ...
  NyasTexDesc(NyasTexType type, NyasTexFmt fmt, int w, int h)
      : Width(w), Height(h), Flags(NyasTexFlags_None), Type(type), Format(fmt),
        MinFilter(NyasTexFilter_Linear), MagFilter(NyasTexFilter_Linear),
        WrapS(NyasTexWrap_Repeat), WrapT(NyasTexWrap_Repeat),
...
typedef struct NyasShaderDesc {
  ...
  NyasShaderDesc(const char *id, int dcount, int tcount, int cmcount,
                 int sdcount, int stcount, int scmcount)
      : Name(id), DataCount(dcount), TexCount(tcount), CubemapCount(cmcount),
        SharedDataCount(sdcount), SharedTexCount(stcount),
        SharedCubemapCount(scmcount) {}
} NyasShaderDesc;
...
typedef struct NyasDrawState {
  ...
  NyasDrawState()
      : BlendSrc(NyasBlendFunc_Default), BlendDst(NyasBlendFunc_Default),
        ...
        EnableFlags(NyasDrawFlags_None), DisableFlags(NyasDrawFlags_None),
        ViewportMinX(0), ViewportMinY(0), ViewportMaxX(0), ViewportMaxY(0),
        ScissorMinX(0), ScissorMinY(0), ScissorMaxX(0), ScissorMaxY(0),
        BgColorR(0.0f), BgColorG(0.0f), BgColorB(0.0f), BgColorA(-1.0f) {}
} NyasDrawState;
...
typedef struct NyasCamera {
  ...
  inline void Init(const NyasIO &io, NyVec3 pos = {0.0f, 2.0f, 2.0f},
                   NyVec3 target = {0.0f, 0.0f, -1.0f}, float far = 300.0f,
                   float fov = 70.0f) {
    Far = far;
    Fov = fov;
    mat4_look_at(View, pos, target, NyVec3::Up());
    mat4_perspective_fov(Proj, to_radians(Fov), io.WindowSize.X,
                         io.WindowSize.Y, 0.01f, Far);
  }
} NyasCamera;
