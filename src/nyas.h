...
struct NyAllocator
{
	static inline void* Alloc(size_t size, void* _ = NULL) {NY_UNUSED(_); return NYAS_ALLOC(size);}
	static inline void Free(void* ptr, void* _ = NULL) {NY_UNUSED(_); NYAS_FREE(ptr);}
};
...
struct NyCircularAllocator
{
	...
	static inline void Free(void* ptr, void* _ = NULL) {NY_UNUSED(_); NY_UNUSED(ptr);}
};
...
struct NyBuffer
{
	...
	inline ~NyBuffer() {A::Free(Data); Data = NULL; Capacity = 0;}
...
struct NyArray
{
	...
	inline void Push(const T& value) {if (Buf.Capacity == Size) {Buf.Reserve(Size > 4 ? Size * 2 : 8);} Buf[Size] = value; ++Size; }
	inline void Pop() {NYAS_ASSERT(Size > 0); --Size;}
...
struct NyVec2
{
	...
	inline void operator=(const NyVec2& other) {X = other.X; Y = other.Y;}
...
typedef struct NyasTexDesc
{
	...
	NyasTexDesc() : Width(0), Height(0), Flags(NyasTexFlags_None), Type(NyasTexType_2D), 
		...
		{BorderColor[0] = 1.0f; BorderColor[1] = 1.0f; BorderColor[2] = 1.0f; BorderColor[3] = 1.0f;}

	NyasTexDesc(NyasTexType type, NyasTexFmt fmt, int w, int h) : Width(w), Height(h), Flags(NyasTexFlags_None), Type(type),
		Format(fmt), MinFilter(NyasTexFilter_Linear), MagFilter(NyasTexFilter_Linear),
		WrapS(NyasTexWrap_Repeat), WrapT(NyasTexWrap_Repeat), WrapR(NyasTexWrap_Repeat)
		{BorderColor[0] = 1.0f; BorderColor[1] = 1.0f; BorderColor[2] = 1.0f; BorderColor[3] = 1.0f;}
} NyasTexDesc;
...
typedef struct NyasDrawState
{
	...
	NyasDrawState() : 
		BlendSrc(NyasBlendFunc_Default),
		BlendDst(NyasBlendFunc_Default),
		Depth(NyasDepthFunc_Default),
		FaceCulling(NyasFaceCull_Default),
		EnableFlags(NyasDrawFlags_None),
		DisableFlags(NyasDrawFlags_None),
		ViewportMinX(0),
		ViewportMinY(0),
		ViewportMaxX(0),
		ViewportMaxY(0),
...
typedef struct NyasCamera
{
	...
	void Navigate();
	inline NyVec3 Eye() const {float inv[16]; mat4_inverse(inv, View); return {inv[12], inv[13], inv[14]};} // Position.
	...
	inline float* OriginViewProj(float out[16])
	{
		...
		out[3] = 0.0f; out[7] = 0.0f; out[11] = 0.0f; out[12] = 0.0f; out[13] = 0.0f; out[14] = 0.0f; out[15] = 0.0f;
		return mat4_multiply(out, Proj, out);
	}
	inline void Init(const NyasIO& io, NyVec3 pos = {0.0f, 2.0f, 2.0f}, NyVec3 target = {0.0f, 0.0f, -1.0f}, float far = 300.0f, float fov = 70.0f)
	{
		Far = far;
		Fov = fov;
